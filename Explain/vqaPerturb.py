...
def VqaPerturbImg(model, valLoader, bNum, positive=True):
    ...
    for i, (img, seq, pad, label) in enumerate(valLoader):
        ...
        for s in range(len(steps)):
            ...
            _, kc = torch.topk(torch.from_numpy(C[:, 0, :50]), k=int((steps[s]) * (Cmask.shape[1])), dim=-1, largest=positive)
            _, kr = torch.topk(torch.from_numpy(R[:, 0, :50]), k=int((steps[s]) * (Rmask.shape[1])), dim=-1, largest=positive)
            _, kroll = torch.topk(torch.from_numpy(Roll[:, 0, :50]), k=int((steps[s]) * (RollMask.shape[1])), dim=-1, largest=positive)
            _, kraw = torch.topk(torch.from_numpy(Raw[:, 0, :50]), k=int((steps[s]) * (RawMask.shape[1])), dim=-1, largest=positive)
